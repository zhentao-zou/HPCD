...
class CLIPEmbedder(AbstractEncoder):
    ...
    def __init__(self,
                 version=None,  # 默认使用本地路径
                 device="cuda",
                 max_length=77):
        super().__init__()
        if version is None:
            # 使用本地的 clip-vit-large-patch14 (从 model/ 往上一级到 step2_autodir/)
            version = os.path.join(os.path.dirname(os.path.dirname(__file__)), "clip-vit-large-patch14")
...
from NAFNet.basicsrA.models.archs.NAFNet_arch import NAFNet
